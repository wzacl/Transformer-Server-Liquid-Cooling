...
class TransformerModel(nn.Module):
    ...
    def forward(self, src, num_steps):
        ...
        for step in range(num_steps):
            ...
            # 僅在推理階段量化
            output = self.soft_quantize(output)
    ...
    def soft_quantize(self, x, temperature=0.1):
        # 使用 sin 函數模擬量化行為，但保持可微分性
        scaled = x * 10
        floor = torch.floor(scaled)
        fractional = scaled - floor
        soft_fractional = torch.sigmoid((fractional - 0.5) / temperature)
        return (floor + soft_fractional) / 10
